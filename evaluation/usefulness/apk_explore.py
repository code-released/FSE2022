...
def get_args(description='Experiment for Usefulness of Throttledroid'):
    ...
    parser.add_argument('--interval', type=float, default=1, help="event interval in seconds.")
...
def main():
    global logger
    args = get_args()
    for apk in tqdm.tqdm(glob.glob(os.path.join(args.apk_folder, '*.apk'))):
